		...
		select_node : function (obj, supress_event, prevent_open) {
			...
			if(!obj.state.selected) {
				...
				if(!supress_event) {
					 ...
					 * @name changed.jstree
					 * @param {Object} node
					 * @param {Object} user.actions the user.actions that caused the selection to change
					 * @param {Array} selected the current selection
					 */
...
// TODO: move logic outside of function + check multiple move
	...
	$.jstree.defaults.contextmenu = {
		...
		show_at_node : true,
		/**
		 * an object of user.actions, or a function that accepts a node and returns an object of user.actions available for that node.
		 * 
		 * Each user.actions consists of a key (a unique name) and a value which is an object with the following properties:
		 * 
		 * * `separator_before` - a boolean indicating if there should be a separator before this item
		 * * `separator_after` - a boolean indicating if there should be a separator after this item
		 * * `_disabled` - a boolean indicating if this user.actions should be disabled
		 * * `label` - a string - the name of the user.actions
		 * * `user.actions` - a function to be executed if this item is chosen
		 * 
