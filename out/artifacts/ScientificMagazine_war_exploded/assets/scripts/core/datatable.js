...
var Datatable = function () {
    ...
    var countSelectedRecords = function() {
        ...
        if (selected > 0) {
            $('.table-group-user.actions > span', tableWrapper).text(text.replace("_TOTAL_", selected));
        } else {
            $('.table-group-user.actions > span', tableWrapper).text("");
        }
    }
    ...
    return {
        ...
        init: function (options) {
            ...
            options = $.extend(true, {
                ...
                dataTable: {
                    "sDom" : "<'row'<'col-md-8 col-sm-12'pli><'col-md-4 col-sm-12'<'table-group-user.actions pull-right'>>r><'table-scrollable't><'row'<'col-md-8 col-sm-12'pli><'col-md-4 col-sm-12'>r>>", // datatable layout
                        ...
                        "success": function(res, textStatus, jqXHR) {
                            ...
                            if (res.sStatus) {
                                if (tableOptions.resetGroupActionInputOnSuccess) {
                                    $('.table-group-user.actions-input', tableWrapper).val("");
                                }
                            }
            ...
            // build table group user.actions panel
            if ($('.table-user.actions-wrapper', tableContainer).size() === 1) {
                $('.table-group-user.actions', tableWrapper).html($('.table-user.actions-wrapper', tableContainer).html()); // place the panel inside the wrapper
                $('.table-user.actions-wrapper', tableContainer).remove(); // remove the template container
            }
